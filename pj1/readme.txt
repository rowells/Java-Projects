                              CSCI 2270  Project 1
                                Sharks and Fish
                      Due 11:55pm Saturday, October 6, 2012

Timeline: You must force yourself to be done with AT LEAST Part 1 by Saturday
	Sept 29th.  If you fall behind, it may be impossible to catch up.
...
and fish breed, eat, and die in the digital ocean.  Each cell of the grid can be
...
ocean at the end of the timestep.  (Don't try to implement a timestep using
just a single Ocean object, you will modify the values of cells whose old values
are still needed to compute the new values for other cells, and you will compute
the wrong answer.)
...
be easier to use for debugging or if you're running on a non-graphical terminal
(like via SSH at home to the CSEL machines).
...
Web), you might be interested to know that they use run-length encoding (along
with other compression techniques) to reduce their sizes.
...
java.util.LinkedList.  However, in CSCI 2270 this is forbidden, because I
want you to be always aware of exactly how your data structures work.
Likewise, you may not use java.util.Vector or other built-in data structures.
...
=========================================================
The last part is possibly the hardest, but it is only worth 10% of the
total score (2 points out of 20), so don't panic if you can't finish it.
...
------------------------
Make sure that your program compiles and runs on the lab machines or class VM
with the autograding program Test.class, which is included with the project
files.
...
Zip these .java files into a single file with your name as always.
For example I would zip into the file John_Black.zip.

Submit this one file to the moodle by the due date.
